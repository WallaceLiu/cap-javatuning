...
//            if (u.isLiveIn(i)) {
        ...
        if (!isStaticCall()) {
            ...
            if (!methodWeaver.isStatic() && f.getLocal(0) == v) {
                ...
                VMType.loadVar(mv, VMType.TOBJECT, methodWeaver.getFiberVar());
...
?     * <pre>
    ...
    private void genSave(MethodVisitor mv, Label saveLabel) {
        ...
        VMType.storeVar(mv, VMType.TOBJECT, stateVar);
        // state.self = this if the current executing method isn't static
        if (!bb.flow.isStatic()) {
            VMType.loadVar(mv, VMType.TOBJECT, stateVar);
        ...
        VMType.loadVar(mv, VMType.TOBJECT, stateVar); // state.pc
        if (pc < 6) {
